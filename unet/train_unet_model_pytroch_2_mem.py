...
def train_unet_pytorch(hdf5_file, output_dir, prev_model_path=None, batch_size=16, num_epochs=500, lr=0.0003, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 0:
            print(f"Epoch {epoch+1}/{num_epochs}: Train Loss = {train_loss:.12f}, Val Loss = {val_loss:.12f}")
...
if __name__ == "__main__":
    ...
    hdf5_files = [f for f in glob.glob(os.path.join(dataset_dir, '*.h5')) if not os.path.basename(f).startswith('bits_')]
    if not hdf5_files:
    ...
    for idx, hdf5_file in enumerate(hdf5_files):
        ...
        if os.path.exists(metrics_file):
            with open(metrics_file, "r") as f:
                metrics = json.load(f)
                train_losses, val_losses = metrics["train_losses"], metrics["val_losses"]
